...
def check_function_definition(func_def, symbol_table: dict):
    ...
    for param in func_def.params:
        # If param.type_annotation is None => default to "int"
        declared = param.type_annotation or "int"
        param_types.append(declared)

    # If the AST has no explicit return_type, we keep None => possible inference or final default
    declared_return_type = getattr(func_def, "return_type", None)
    logger.debug(f"Declared/initial return type for '{func_name}': {declared_return_type}")
    ...
    # Also store the current function's return type in a special key
    ...
    # Step 4) After checking all statements, fetch the possibly updated return type
    final_return_type = local_scope.get("__current_function_return_type__", None)

    # If final_return_type is still None, you can decide to default to "void"
    if final_return_type is None:
        final_return_type = "void"
        logger.debug(f"No return statements found; defaulting function '{func_name}' to 'void'.")
