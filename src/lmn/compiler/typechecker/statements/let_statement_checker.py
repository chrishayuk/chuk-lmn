...
class LetStatementChecker(BaseStatementChecker):
    ...
    def check(self, stmt) -> None:
        ...
        if expr:
            ...
            expr_type = self.dispatcher.check_expression(expr, declared_type)
            ...
            if declared_type:
                    ...
                    if expr_type == "array":
                        ...
                        if hasattr(expr, "elements"):
                            for element_expr in expr.elements:
                                ...
                                if not elem_type:
                                    elem_type = self.dispatcher.check_expression(element_expr)
                ...
                if expr_type == "array":
                    if hasattr(expr, "elements") and expr.elements:
                        elem_type = self.dispatcher.check_expression(expr.elements[0])
                        for e in expr.elements[1:]:
                            e_type = self.dispatcher.check_expression(e)
